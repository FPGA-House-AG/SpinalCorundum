...
object PreventReplayMN {
  ...
  def main(args: Array[String]) {
    val verilogReport = Config.spinal.generateVerilog({
      val toplevel = new PreventReplayMN(10, 64, 1024, 32, 4)
      toplevel
    })
...
{
  val windowSize = m * n
  val rejectAfterMessages = ((1 << counterWidth - 1) - (m-1)*n)
...
object PreventReplayMNSim {
  def main(args: Array[String]) {
    SimConfig.withFstWave.doSim(new PreventReplayMN(10, 64, 1024, 32, 4, initMem = true)){dut =>
